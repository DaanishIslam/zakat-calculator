...
export class CalculationsService {
  ...
  private calculateTotalAssets(
    ...
    // total += this.convertToDefault(formData.preciousStones, formData.cashCurrency);
    
    // Financial Assets
    total += this.convertToDefault(formData.cashInHand, formData.cashCurrency);
    total += this.convertToDefault(formData.bankBalance, formData.bankCurrency);
    total += this.convertToDefault(formData.loansReceived, formData.loanCurrency);
    total += this.convertToDefault(formData.investmentsGross, formData.investmentCurrency);
    total += this.convertToDefault(formData.ppfGross, formData.ppfCurrency);
    total += this.convertToDefault(formData.cryptoGross, formData.cryptoCurrency);
    
    // Business Assets
    total += this.convertToDefault(formData.businessShares, formData.sharesCurrency);
    total += this.convertToDefault(formData.deadStocks, formData.deadStocksCurrency);
    total += this.convertToDefault(formData.creditSales, formData.creditSalesCurrency);
    
    // Real Estate
    total += this.convertToDefault(formData.landValue, formData.landCurrency);
    total += this.convertToDefault(formData.rentalRevenue, formData.rentalCurrency);
    
    // Partnership
    total += this.convertToDefault(formData.firmCapital, formData.firmCurrency);
    total += this.convertToDefault(formData.firmProfit, formData.firmProfitCurrency);
    
    // Agriculture Calculations
    if(formData.agriRainValue) {
      total += formData.agriRainValue * 0.10 * // 10% rate
        this.convertToDefault(1, formData.agriRainCurrency);
    }
    if(formData.agriIrrigationValue) {
      total += formData.agriIrrigationValue * 0.05 * // 5% rate
        this.convertToDefault(1, formData.agriIrrigationCurrency);
    }
    if(formData.agriHybridValue) {
      total += formData.agriHybridValue * 0.075 * // 7.5% rate
        this.convertToDefault(1, formData.agriHybridCurrency);
    }
    
    // Livestock (assuming animalAmount is in SAR)
    total += this.convertToDefault(formData.animalPrice, formData.animalCurrency);
  ...
  private calculateTotalLiabilities(formData: FormData): number {
    ...
    // Debts and Loans
    total += this.convertToDefault(formData.creditSuppliers, formData.creditCurrency);
    total += this.convertToDefault(formData.badDebts, formData.badDebtsCurrency);
    total += this.convertToDefault(formData.partnershipWithdrawals, formData.partnershipCurrency);
    total += this.convertToDefault(formData.loansFromFriends, formData.friendsLoanCurrency);
    total += this.convertToDefault(formData.loansFromBanks, formData.bankLoanCurrency);
    total += this.convertToDefault(formData.firmLoan, formData.firmLoanCurrency);
